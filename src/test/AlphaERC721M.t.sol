...
import {IAlignmentVault} from "../contracts/interface/IAlignmentVault.sol";
...
contract AlphaCrate721MTest is TestWithHelpers, ERC721Holder {
    ...
    function testInitialize() public {
        ...
        assertEq(IAlignmentVault(manualInit.alignmentVault()).alignedNft(), address(nft));
        require(manualInit.owner() == address(this));
        require(keccak256(abi.encodePacked(manualInit.name())) == keccak256(abi.encodePacked("Crate721M Test")));
        require(keccak256(abi.encodePacked(manualInit.symbol())) == keccak256(abi.encodePacked("Crate721M")));
        require(
    ...
    function testName() public view {
        require(keccak256(abi.encodePacked(template.name())) == keccak256(abi.encodePacked("Crate721M Test")));
    }

    function testSymbol() public view {
        require(keccak256(abi.encodePacked(template.symbol())) == keccak256(abi.encodePacked("Crate721M")));
    }
    ...
    function testMintRevertMintCapReached() public {
        template.unpause();
        template.mint{value: 0.01 ether * 100}(address(this), 100, address(0));
    ...
    function testReceive() public {
        template.unpause();
        (bool successMint,) = payable(address(template)).call{value: 1 ether}("");
        require(successMint);

        (bool success,) = payable(address(template)).call{value: 5 ether}("");
        require(success);
        require(address(template.alignmentVault()).balance == 1.2 ether);
    }
    ...
    function testOnERC721ReceivedRevertNotAligned() public {
        vm.expectRevert(ICrate721M.NotAligned.selector);
